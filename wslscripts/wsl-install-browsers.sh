...
generate_report() {
  ...
  
  {
    ...
    for browser in google-chrome-stable google-chrome-beta google-chrome-unstable \
      ...
      local display_name="$browser"
      local check_name="$browser"
      ...
      if command -v "$check_name" &>/dev/null || [[ "$check_name" == "opera" && $(command -v opera 2>/dev/null) ]]; then
        status="✓"
        status_color="\e[32m"  # Green for success
        path=$(command -v "$check_name" || command -v opera)
        version=$(get_version "$check_name")
        install_date=$(stat -c %y "$path" 2>/dev/null | cut -d' ' -f1 || echo "Unknown")
      fi
      
      printf "| %-25s | ${status_color}%-10s\e[0m | %-15s | %-50s | %-20s |\n" "$display_name" "$status" "$version" "$path" "$install_date"
    done
    print_table_border 150 "-" "\e[1;34m"
...
install_packages() {
  ...
  for pkg in "${packages[@]}"; do
    ...
    (
      if [[ "$pkg" == "opera-stable" ]]; then
        # Special handling for Opera with improved installation method
    ...
    if wait $pid; then
      ...
      if [[ "$pkg" == "opera-stable" ]]; then
        print_red "✗ Failed to install Opera"
...
main() {
  ...
  if [ "$UPGRADE_ONLY" = true ]; then
    ...
    install_packages google-chrome-stable google-chrome-beta google-chrome-unstable \
                    microsoft-edge-stable microsoft-edge-beta microsoft-edge-dev \
                    opera-stable brave-browser firefox konsole pulseaudio dos2unix code
