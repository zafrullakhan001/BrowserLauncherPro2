  ...
  for pkg in "${packages[@]}"; do
    ...
    (
      if [[ "$pkg" == "opera" ]]; then
        ...
        DEBIAN_FRONTEND=noninteractive timeout $timeout_seconds sudo apt-get install -y "$pkg" >/dev/null 2>&1
      else
    ...
    if wait $pid; then
      ...
      if [[ "$pkg" == "opera" ]]; then
        print_red "✗ Opera installation timed out after $timeout_seconds seconds"
